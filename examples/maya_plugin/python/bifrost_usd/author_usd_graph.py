...
def insert_prim_node(graph_selection: GraphEditorSelection, new_node_def: dict) -> str:
    ...
    graphAPI.set_param(
        newNode,
        (primPathParamName, primPath),
        current_compound=graph_selection.currentCompound,
    )
    return newNode
...
def resolve_prim_path(
    ...
    parentPath = graphAPI.param(add_to_stage_path, portName)
...
def insert_maya_variant(graph_selection: GraphEditorSelection) -> str:
    ...
    if not sel.nodeSelection:
        ...
        if not sel.nodeSelection:
            cmds.warning(
                "You must select a node upstream (with an 'out_stage' output port) in the Bifrost Graph Editor"
            )
            return ""
    ...
    if mayaSelection:
        ...
        return add_one_maya_selection_as_variant_to_stage(
            parentPath, variantSetName, mayaPath, sel
        )
    else:
        cmds.warning(
...
if __name__ == "__main__":
    # insert_maya_variant(GraphEditorSelection("", "", "", "out_stage", []))
    # graphAPI.auto_layout_selected_nodes()
    ...
    with CurrentMayaSelection(cmds.ls(selection=True)):
        ...
        add_maya_selection_as_variants_to_stage(
            "/Model", "VSET"
        )
    # get_graph_selection()
